...
function quiz_handle_form_configureaweber() {
    ...
    global $wpdb;
    if (isset($_POST['skip_optin_perma'])){
        $ar = get_option('vc_skipped_optin');
        if(!$ar)$ar= array();
        if(!in_array($_GET['quizid'], $ar)){
            $ar[]=$_GET['quizid'];
           update_option('vc_skipped_optin', $ar);
        }
    ...
    if ($q->save()) {
        ...
        $wpdb->update(QUIZ_DB_TABLE_QUIZ, $data, array("id" => mysql_escape_string($_GET['quizid'])));
